...
async function save(client, instance, customKey, ttl = undefined) {
  if (!instance) {
  ...
  return client.set(key, instanceToData(instance), ttl).then(() => instance)
}

function saveAll(client, model, instances, customKey, ttl = undefined) {
  const key = [model.name, customKey]

  return client.set(key, instances.map(instanceToData), ttl).then(() => instances)
}
